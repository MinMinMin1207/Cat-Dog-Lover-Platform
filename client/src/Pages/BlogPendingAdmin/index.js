...
import styles from "./BlogPendingAdmin.module.scss";
...
function BlogPendingAdmin() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await axios.get("http://localhost:3000/blogs/pending_blog");
  ...
  const handleAccept = async (id) => {
    try {
      const res = await axios.put("http://localhost:3000/blogs/accept_blog", {
        id: id,
      });
      if (res) {
        console.log("successfully");
        window.location.href = "/blogacceptadmin";
      } else {
        console.log("err");
      }
    } catch (err) {
  ...
  const handleReject = async (id) => {
    try {
      const res = await axios.put("http://localhost:3000/blogs/reject_blog", {
        id: id,
        note: noteReject,
      });
      if (res.data.code === 200) {
        console.log("successfully");
        window.location.href = "/blogrejectedadmin";
      }
    } catch (err) {
      console.log(err);
    }
    ...
    <div className={cx("pending-wrapper")}>
      ...
      <div className={cx("pending-post")}>
        <div className={cx("pending-header")}>
          <h1 className={cx("heading")}>Pending Blog</h1>
        </div>
        {list.length > 0 ? (
          <table className={cx("table")}>
            <thead className={cx("table-header")}>
              <tr>
                <th className={cx("table-item", "price")}>ID</th>
                <th className={cx("table-item", "name")}>Author</th>
                    ...
                    <td className={cx("table-item", "approve")}>
                      <div className={cx("list-approve")}>
                        <Link to="/previewblog" target="_blank">
                          ...
                          >
                            <img src={images.viewIcon} className={cx("img")} />
                          </div>
                        </Link>
                        <div
