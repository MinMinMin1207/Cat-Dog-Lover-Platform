...
import styles from "./AvailableStaff.module.scss";
...
function AvailableStaff() {
  ...
  const handleClickReject = async (id) => {
    // handleClickOpen();
    console.log(id)
    const res = await axios.delete(`http://localhost:3000/users/remove_staff/${id}`)
    if(res.data.code === 200) {
      window.location.href = "/availablestaff";
    }
  };
        ...
        <table className={cx("table")}>
          <thead className={cx("table-header")}>
            <tr>
              ...
              <th className={cx("table-item", "banUser")}>Remove</th>
            </tr>
          </thead>
