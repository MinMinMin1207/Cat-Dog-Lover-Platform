...
import styles from "./BlogRejectedAdmin.module.scss";
...
function BlogRejectedAdmin() {
  const [list, setList] = useState([]);

  useEffect(() => {
    async function fetchData() {
      const res = await axios.get(`http://localhost:3000/blogs/rejection_blog`);
    ...
    <div className={cx("pending-wrapper")}>
      <div className={cx("pending-post")}>
        <div className={cx("pending-header")}>
          <h1 className={cx("heading")}>Rejected Blog</h1>
        </div>
...
export default BlogRejectedAdmin;
